...
**Preview**:
A Host launched from a Worktree as a separate process, so a change to varnick itself can be tried before it lands. Its own window, its own Door, its own port; it sees the same Sessions as Live.
