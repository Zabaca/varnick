...
Today it arrives at the *start of the next Turn*, because a Turn is the only thing that drains the wire — late, and not lost, which is the whole of the defect. Making it immediate means Core polling while idle, which is a second consumer of that queue and a race at the boundary if it is added carelessly.
