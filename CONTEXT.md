...
**Projection**:
A file varnick generates *for* the confined tree by reading something the confined tree may not read, carrying only the fields that were argued for and nothing else. There is one: `.varnick/gitconfig`, written at launch by the unconfined runtime from the developer's real git config, carrying `user.name` and `user.email`.

The word is doing real work, so it is worth the entry. A **copy** would be the developer's config with the unreadable parts removed; a Projection is a new file with the permitted parts put in — which is the difference between a denylist and an allowlist, and the whole of why this one is safe. A gitconfig is *executable configuration*: `core.hooksPath`, `alias.*` and `credential.helper` beginning `!`, `filter.*.clean` and the rest all run commands, so a copy would hand the agent whatever the developer happened to have. `EXECUTING_GIT_KEYS` in `packages/harness/src/gitconfig.ts` enumerates them once.

It exists because git treats a global config it can see and cannot read as **fatal**, and the **Sandbox** denies `$HOME` — so every git command inside it exited 128 until `GIT_CONFIG_GLOBAL` was pointed at this file, taking `git worktree add` and therefore the whole of [ADR-0014](./docs/adr/0014-core-is-authored-in-a-worktree.md) with it. It is denied to the agent in both senses that matter: `denyWrite` in the live tree, and `PROTECTED_PATHS` so a force-added copy cannot be landed unattended.
_Avoid_: copy (the opposite of what it is), mirror, shim, synced config
