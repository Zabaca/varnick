...
- **Core and Userspace are different spaces, enforced by the sandbox policy.** The agent writes Userspace freely and cannot write `packages/core/**`, `packages/harness/**`, `src-tauri/**`, `vite.config.*`, `package.json`, `scripts/**`, `sandbox-policy.json`, its baseline, `.git/hooks/**`, `.git/config*`, `.githooks/**` or `.varnick/gitconfig` — see [ADR-0002](docs/adr/0002-core-userspace-boundary.md). The policy entries are on the list for the same reason as the code: an agent that can rewrite the generator, the policy, or the baseline it is compared against can widen its own fence on the next launch. `scripts/**` and the two `.git` entries close the same shape one step out — deny a file the host executes but leave the file *it* invokes writable, and the deny does nothing ([ADR-0016](docs/adr/0016-gits-own-directory-is-outside-the-review-path.md)). `.githooks/**` is that shape a third time and the one that reads as a contradiction: the directory is tracked *and* denied, because tracked says a hook can be reviewed rather than that it was — hooks are authored in a Worktree like Core, and ADR-0016's amendment is where that is argued. `.varnick/gitconfig` is the shape a fourth time and the only entry varnick creates itself: git fatals on a global config it cannot read and `$HOME` is denied, so every git command in the Sandbox is pointed at that file — leaving it writable would hand the agent `core.hooksPath` on the developer's next commit. It is a *projection* of the developer's identity and nothing that executes; `packages/harness/src/gitconfig.ts` argues the allowlist. Adding a Surface must therefore never require editing Core; discovery is filesystem-based.
