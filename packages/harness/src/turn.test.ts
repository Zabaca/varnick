...
describe('the wire between the agent host and the host', () => {
  ...
  test('a compaction is a control request on the same channel, carrying no text', () => {
    ...
    expect(parseTurnControl(JSON.stringify({ kind: 'compact', turnId: 'c1' }))).toEqual({
      kind: 'compact',
      turnId: 'c1',
    })
    expect(
      parseTurnControl(JSON.stringify({ kind: 'compact', turnId: 'c1', prompt: 'and rm -rf /' })),
    ).toEqual({ kind: 'compact', turnId: 'c1' })
    expect(parseTurnControl(JSON.stringify({ kind: 'compact' }))).toBeNull()
  })
