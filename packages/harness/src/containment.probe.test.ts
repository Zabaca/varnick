...
/*
  ...
  2. **Nothing here weakens the policy to make a result tidier.** Where the
     ...
     is recorded and the policy is left alone — see the DENIED_BINARIES matrix,
...
// ---------------------------------------------------------------------------
// 2. Every entry in DENIED_BINARIES, measured
// ---------------------------------------------------------------------------
...
test.skipIf(blocked !== null)(
  'every entry in DENIED_BINARIES is measured, and three of the four still execute',
  async () => {
    /*
      ...
      `srt` has no execute allowlist, so `DENIED_BINARIES` blocks a binary by
    ...
    */
    ...
    report('probe 2 — DENIED_BINARIES: read is denied, execution mostly is not', rows)
...
test.skipIf(blocked !== null)(
  ...
  async () => {
    /*
      ...
      A throwaway clone gets the same policy with `DENIED_BINARIES` lifted out of
      ...
      const variant = {
        ...
        filesystem: {
          ...
          denyRead: base.filesystem.denyRead.filter(
            (path) => !(DENIED_BINARIES as readonly string[]).includes(path),
          ),
        },
      }
