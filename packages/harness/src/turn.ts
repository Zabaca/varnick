 ...
 *
 * Three, and each one earns its place. The agent host is a Claude Code session
 * inside the Sandbox; every additional thing it can be asked to do is another
 * thing something outside the Sandbox can make it do.
 *
 * `compact` is the one added since, and it is the ADR-0003 argument rather than
 * a convenience: summarising a conversation is a model call on *this* session,
 * and the obvious implementation — `query()` on the host — would put a second
 * Claude Code process outside `srt`. It carries a Turn id and nothing else.
 * There is no prompt on it to smuggle anything through, because the prompt is a
 * constant this module owns ({@link COMPACT_COMMAND}).
 */
