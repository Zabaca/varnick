...
## Amendment, 2026-09-18: off means inherited

With the Proxy off, a Session inherits the Host's environment untouched, credential variables and `ANTHROPIC_BASE_URL` included. The first amendment had the Host strip them in both modes, and in the off mode that stripping protects nothing and breaks the one setup the developer actually runs: a shell that sourced `zabaca/claude-mitm-proxy`'s client env carries `HTTPS_PROXY`, the CA paths and this machine's fleet placeholder, so `claude` launched from it reaches Anthropic through the fleet proxy and the real token never touches the machine. Stripping the placeholder sent the agent to that proxy with no credential. Proxy on still strips and overrides, because that is the mode with a boundary to keep.
