...
- **This is fault isolation and not privilege isolation** — [ADR-0021](./0021-userspace-shares-cores-realm.md). A Surface that fails cannot take Core down; a Surface that *runs* is code in Core's realm, with Core's globals and Core's bridge to the host. The dynamic import is what decides when a module is evaluated, not what it may touch afterwards, so nothing here is a trust boundary and no lint rule can make it one.
