...
**The store was chosen partly on a claim that turned out to be false** *(stage 6)*. The Sandbox denies the agent read on `/usr/bin/security`, and that was taken to mean the agent could not run it. Measured under the real policy, it runs — srt allows `process-exec` unconditionally — and it reaches securityd over Mach, which no filesystem denial covers. A command under the policy read a stored value back in plaintext. The correction and what closing it would take are in [ADR-0003](../../docs/adr/0003-containment-wraps-the-process-tree.md); the measurement is an inverted assertion in `packages/harness/src/sandbox.boundary.test.ts`.
