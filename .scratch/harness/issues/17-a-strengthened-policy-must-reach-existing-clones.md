...
- [ ] A clone whose policy predates a strengthening learns about it, without its own edits being discarded
- [ ] What varnick reports distinguishes "you changed this" from "we changed this"
- [ ] A policy weaker than the current generator's in a way that matters is surfaced where a developer will see it, not only in a file
- [ ] `packages/harness/src/sandbox.boundary.test.ts` passes without anyone deleting a generated file first — that is the regression this ticket is named after
- [ ] Moving a clone between machines, or between home directories, is not reported as tampering
