...
Fredrin owns the worktree's lifecycle for you. Completion reclaims eligible
worktrees; **archiving** also clears merged/closed PR associations in Fredrin and
requests cleanup on each owning machine. GitHub history and open PRs stay.
Archive cleanup removes only proven clean worktrees, preserving branches and a
small resume folder. Dirty, unpushed, unknown-owner or unknown-ignored-content
worktrees are retained and logged; known ignored bootstrap files and caches
(including dotenv files) are disposable. Offline machines retry on their next
cleanup pass. Restoring a ticket does not recreate cleared PR associations or a
checkout. Explicit ticket/worktree deletion remains a separate action.
...
3. **Blocked** — the Worker needs a decision before it can continue. A human
   can park a ticket here, but so does the runtime's own ask tool (see below),
   and the Worker declares it itself with `ticket blocked`.
...
**Declare how your turn ends.** Two verbs, two different claims, and they are the
only two:

- `./.fredrin/fredrin ticket finish '{...}'` — **I am done.** Records your
  acceptance checks, pushes the branch, opens the PR and moves the ticket to Review.
- `./.fredrin/fredrin ticket blocked "<why you need a human decision>"` — **I need
  a decision.** Free prose, not JSON: everything after the verb is the reason. Use it
  for any question only a human can answer, *including* "I finished my part and you
  have to choose" — there is nothing wrong in that case, which is exactly why it is
  not `ticket error`. `error` claims something BROKE, and stamps a red
  `error` label on the ticket to say so.

  It records the declaration on this run; it does not move the ticket or reach the
  human by itself. Ask through your runtime's picker as well, below.

**Need the human before you can continue? Use the input path your runtime
actually exposes and permits.** In **Claude Code**, call `AskUserQuestion`:
its question hook parks the ticket in **Blocked**, rings the attention bell,
and keeps this session live so the human answers in place. Keep using that
picker rather than ending a Claude turn on a bare prose question.

In **Codex**, call the synchronous `request_user_input` tool: it is Codex's
multiple-choice picker, and Fredrin enables it for this session. Use it for
every question the human must answer — clarifications, required decisions and
sign-off gates — with 2–3 options each. For a clarification put your recommended
option first; at a sign-off gate put the safe, non-approving option first and
recommend nothing, because the picker pre-highlights row 1 and a bare Enter
submits it. Its question hook parks the ticket in **Blocked** and the turn waits
for the answer; wait for the actual reply before doing any work behind a
required decision or sign-off gate. Never use `request_user_input_async`: it
draws no picker and returns before an answer. Only if the tool reports it is
unavailable, ask one clear question in your final terminal response and end the
turn awaiting a typed reply. Tell the human to reply in this terminal, then put
the question on the final line ending in a literal question mark (?). Continue
this session when they answer; do not retry an unavailable tool. A board-status
change is not itself human sign-off.

For an explicit yes/no sign-off decision, offer exactly two options: "Not yet"
first, then "Approve"; recommend neither. Use a workflow-specific approving label
when supplied, such as "Start building". Give each a one-sentence description
of what that choice means for the specific proposal. A "Not yet" answer leaves
that proposal unapproved; continue only independent work or address the feedback.
Do not invent an approval gate when the human has already authorized the work.
Never use `request_user_input` for permission requests to execute tools or
escalate sandbox access; use the runtime permission mechanism. A plan or design
sign-off is a human decision and uses the picker, but cannot grant runtime tool
permissions.

If the call returns `{"answers":{}}`, nobody answered: Codex closes an
untouched picker on its own after about two minutes. Call it again once; if it
returns empty again, restate the question and its options in your final
terminal response with the final line ending in a literal question mark (?)
and end the turn. Fredrin keeps the ticket Blocked either way; never proceed on
an empty answer.

**Before asking for UI design approval in Codex, attach the visual to the ticket.**
This includes design proposals within code tickets: create a self-contained HTML
mockup or an image/PDF preview, upload it with
`./.fredrin/fredrin ticket upload <file> --label "<what the human is reviewing>"`,
verify the attachment succeeded, then reference its artifact labels and URLs in
the approval question. Reuse an attached preview if it matches the proposal.
Resolve an upload failure or report the upload blocker before asking for design
approval; saving `ticket.plan` alone is not a visual handoff. Preview preparation
may happen before sign-off; keep application implementation paused. Ordinary
clarification questions and nonvisual plans need no mockup, and this does not
introduce a new approval requirement.

For every runtime, posting only a `ticket comment` or asking and then continuing
dependent work is not waiting. Elapsed time, a default selection, an empty
response, and your recommendation are not human sign-off. The build seed's
runtime-specific guidance also applies to older copies of this manual that
name only Claude's tool or prefer Codex's async tool.
...
| run the app / start the dev server / spin up localhost | `fredrin servers start <id>` (or `--all` for every server); list with `fredrin servers list`, stop with `fredrin servers stop`. These live in the **Dev** tab |
...
- **"start" / "run" is ambiguous — read the object.** "start / run *the ticket /
  ...
  the **Dev** tab. Never conflate the two.
...
14-section ordinary plan described below, naming exact touchpoints (repo-relative
...
For a NEW ordinary ticket plan, reply in markdown with these 14 mandatory H2 sections, in this exact order and with these exact names:
## Objective — What outcome are we trying to achieve, and for whom? Name a falsifiable end state.
## Current behavior — What happens today? Include evidence and known code paths.
## Desired behavior — What must happen after the change? Include key user/system flows.
## Implementation plan — The ticket's timeline: its ordered work steps (research, each code change, docs, verification) as a GFM checklist — `- [ ]` bullets in execution order, never `1.` — 3–12 steps, one per line, each ticked the moment its work is done; notes ride as indented sub-bullets.
## Acceptance criteria — Observable, testable conditions defining success, as a second GFM checklist: 2–6 `- [ ]` items, at least one a concrete runnable or observable check. One part of progress beside the Implementation plan steps, not the whole of it. Check ticket-specific behavior, never generic typecheck/lint/test gates; those are separately selected verification steps.
## Scope — What components and behaviors are included?
## Non-goals — What must not be changed or solved in this ticket?
## Touchpoints — Expected files, modules, APIs, schemas, infrastructure, tests, and dependencies. Name known repo-relative paths in backticks with a short note.
## Constraints — Architecture, compatibility, security, performance, cost, and policy boundaries.
## Edge cases and failure behavior — Invalid inputs, empty states, retries, partial failure, concurrency, and fallback behavior.
## Validation plan — Automated tests, manual checks, commands, screenshots, benchmarks, and required evidence. Include only applicable verification; never claim an unrun check passed.
## Risks — Likely regressions, uncertainty, blast radius, and mitigations.
## Rollout and rollback — Feature flags, migrations, deployment order, monitoring, and reversal strategy.
## Open questions — Unresolved decisions, owner, and whether each blocks implementation. Write None when there are no unresolved decisions.
Keep every section separate; never drop or merge one. Be concise. If a section does not apply, keep its heading and explain why briefly. Label assumptions explicitly. When evidence or a code path is unavailable, say what is unknown and who must resolve it rather than inventing facts. Do not present unconfirmed suggestions as settled decisions.
Implementation plan and Acceptance criteria are the plan's two live checklists; add no other. Preserve existing Acceptance checks or Action items headings when editing an older plan, and leave its prose Implementation plan as prose unless asked to restructure. Keep checkbox text and state unless the requested change requires new items.
For an existing plan, preserve its organization and unrelated content unless the human asks to restructure it; in that case use the 14-section format. An existing specialized app, report, or theme-starter delivery contract takes precedence: do not rewrite it to fit this ordinary plan format.
Planning depth and custom depth instructions govern detail and discussion, never omission of required sections in a new ordinary plan. A Quick plan still has all 14 sections, using concise stated assumptions or unknowns where needed. No preamble or closing remarks.
...
- `./.fredrin/fredrin ticket update-plan <<'PLAN'` … `PLAN` — save the implementation plan
  from **stdin** (preferred; no JSON escaping, no temp file). Use markdown with two
  GFM checklists (`- [ ]` / `- [x]`): the `## Implementation plan` work steps and
  the `## Acceptance criteria` (older `## Acceptance checks` and `## Action items`
  headings remain supported). Do NOT `jq` a temp file or
  hand-build `'{"plan":"..."}'` — a blocked or partial temp-file write makes jq
  slurp stale content and saves the wrong plan.
- `./.fredrin/fredrin ticket check <id>` / `ticket uncheck <id>` — flip ONE action item
  without re-sending the whole plan. Cheap and atomic, so during **Build** flip each
  item the moment you finish it — progress then updates in real time. `<id>` is the
  item's stable id, listed beside it in `ticket get`'s `planChecklist`; its full text
  or a unique fragment works too, and a 1-based number still works. **Prefer the id**:
  editing the plan renumbers every position below the edit, so a number you read
  earlier can now name a different item. An address that matches two items is refused,
  never guessed at. (Also: `plan check <id>` / `plan uncheck <id>`.)
...
- `./.fredrin/fredrin ticket blocked "<why you need a human decision>"` — declare that you
  need a decision only a human can make. Free prose, not JSON. Distinct from `ticket error`:
  that one claims something went wrong, this one claims a decision is owed — which includes
  "I finished my part and you have to choose".
...
(`./.fredrin/fredrin ticket update-plan`, all 14 ordinary plan headings, a `- [ ]`
step under Implementation plan and one `- [ ]` acceptance criterion; state unknowns
until researched), then re-save it as you learn — the human watches it
fill in. Rewriting the whole plan never ticks or unticks a box; only
`ticket check <id>` does — and it changes item ids, so re-read them afterwards.

Once it has one, the plan carries two live checklists. The **Implementation
plan** is the ticket's timeline — the ordered work steps, ticked as each one's
work lands. The **acceptance checks** are the contract — its definition of done,
ticked as each one is verified. Neither is a list flipped (if ever) at the very
end; together they are the progress the board shows:

- **Tick the timeline as you go.** The moment a step's work exists — the file is
  changed, the doc is written, the command has run — `./.fredrin/fredrin ticket check <id>`
  it (`ticket get`'s `planChecklist` marks a step `section: implementation`). A
  step is done when the work is done, not when a check passes. Do not reword a
  step you have already ticked — a reworded item comes back unticked; add a new
  step instead.
- **Review the acceptance checks upfront.** Before writing any code, read every acceptance check
...
- **Validate with evidence, tick as you go.** The moment you finish an item,
  ...
  evidence, never self-graded prose — then immediately `./.fredrin/fredrin ticket check <id>`
  it (`<id>` is the stable id `ticket get` lists beside the item, and it does not move
  when you edit the plan the way a position does). Do this one item at a time as
  you go, never batched at the end, so the board shows real-time progress. Only
  ever check an item you have positively verified. Nothing blocks on these boxes,
  which is exactly why they have to be honest: the board shows your `n/N` live
  and `finish` reports whatever is still open, so an unticked box is what the
  reviewer reads as work you did not do.
...
- **Blocked on a decision** → `./.fredrin/fredrin ticket blocked "<why>"`, then put the
  question to the human through your runtime's picker and wait for the answer.
...
**This repo has no concepts layer and no notes layer.** Durable domain knowledge
is the glossary plus the ADRs: a term belongs in `CONTEXT.md`, a decision in
`docs/adr/`. Do not create `docs/concepts/` or `docs/notes/`, and do not file a
concept doc — if `context-classify` wants one, the term goes in `CONTEXT.md`
instead. There is no `AGENTS.md`; the agent-facing entry is `CLAUDE.md`.

**Tickets are Fredrin tickets.** `docs/agents/issue-tracker.md` is the operating
manual: a spec is a Goal, work is tickets under it, and nothing is filed as a
GitHub issue or as a markdown file in the repo. Read `CLAUDE.md` before starting.
